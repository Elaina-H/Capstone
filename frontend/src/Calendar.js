...
const Calendar = ({ activeDay, setActiveDay, eventsArr, addEvent, tasksArr, addTask, deleteEventAndTask }) => {
  const months = [
    "January",
    "February",
    "March",
  ...
  const initCalendar = () => {
    ...
    for (let day = 1; day <= lastDate; day++) {
      ...
      const dayClass = `day ${isToday ? "today" : ""} ${eventForDay ? "has-event" : ""}`;
      daysArr.push(
        <div
          key={`day-${day}`}
          className={dayClass}
    ...
    const eventForDay = eventsArr.some(
      (event) =>
        event.Day === days && event.Month === month + 1 && event.Year === year
    );
...
/* allows Calendar to display next month on button press */
  ...
  const getRoomCodeFromDB = async (roomCode) => {
    try {
      // Query to get RoomID from the "Rooms" table using RoomCode
      const roomDetails = await axios.get(`http://127.0.0.1:8000/api/rooms/${roomCode}/`);
      const room = roomDetails.data.RoomID;
      return room;
  ...
  const addNewEvent = async () => {
    ...
    const room = await getRoomCodeFromDB(code);
...
// add task sequence still needed here
      addEvent(newEvent);
      setEventName("");
      setEventTimeFrom("");
      setEventTimeTo("");        
...
/* below is actual Display */
  return (
    <div className="container">
      ...
      <div className="left">
        ...
        <div className="calendar">
		  ...
		  <button className = "scr-library" onClick={() => window.scrollTo({ top: 1470, behavior: 'smooth' })}> Scroll to view Library rooms</button>
        </div>
      </div>
    ...
    <div className="event-dropdowns">
          ...
          <div className="dropdown">
            <button className="dropbtn">Events for Displayed Month</button>
            <div className="dropdown-content">
